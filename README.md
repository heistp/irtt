...
To build IRTT from source, you must:
...
- [Install Go](https://golang.org/dl/) (Note: If you're installing Go for the
  ...
  set `$GOROOT`.)
- Install irtt by doing `go get -u github.com/peteheist/irtt/cmd/irtt`. This
  should place the `irtt` executable in `$HOME/go/bin`, or `$GOPATH/bin` if you have
  `$GOPATH` defined.

_Note:_ If you want to build the source for development, you must install:

- The `stringer` utility, by doing `go get -u -a golang.org/x/tools/cmd/stringer`.
  ...
  used.
- The `pandoc` utility for generating man pages and HTML documentation from
  their markdown source files. This can be installed with `apt-get install
  pandoc` on Debian flavors of Linux or `brew install pandoc` on OS/X. See the
  [Pandoc](http://pandoc.org/) web site for more information.
